...
if(VCPKG_TARGET_IS_OSX)
    set(_qt_find_wrap_opengl "${CURRENT_PACKAGES_DIR}/share/Qt6/FindWrapOpenGL.cmake")
    if(EXISTS "${_qt_find_wrap_opengl}")
        file(READ "${_qt_find_wrap_opengl}" _qt_find_wrap_opengl_contents)
        set(_qt_find_wrap_opengl_original "${_qt_find_wrap_opengl_contents}")

        # Qt's upstream script adds AGL as a fallback on macOS.
        # On current SDKs this causes qtshadertools/qsb to fail linking.
        string(REPLACE
            ...
            "target_link_libraries(WrapOpenGL::WrapOpenGL INTERFACE \${__opengl_fw_path})"
            _qt_find_wrap_opengl_contents
            "${_qt_find_wrap_opengl_contents}"
        )

        if(_qt_find_wrap_opengl_contents STREQUAL _qt_find_wrap_opengl_original)
            message(FATAL_ERROR "Failed to patch ${_qt_find_wrap_opengl}: expected AGL fallback block not found.")
        endif()

        file(WRITE "${_qt_find_wrap_opengl}" "${_qt_find_wrap_opengl_contents}")
    endif()
endif()
